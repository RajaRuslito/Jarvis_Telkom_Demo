...
const MainPage = () => {
  ...
  const menu = [
    ...
    { id: 2, title: 'Job Requirement', endpoint: 'jr', description: 'This is the job requirement of the company' },
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/${activeMenu.endpoint}/`);
        if (!response.ok) {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:5000/${activeMenu.endpoint}/upload-xlsx`, formData, {
  ...
  const fetchJobs = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/${activeMenu.endpoint}/all/search?search=${searchTerm}`
      );
  ...
  const handleFileDownload = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/${activeMenu.endpoint}/all/download`, {
        responseType: 'blob',
      });
  ...
  const handleFileDownloadTemplate = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/${activeMenu.endpoint}/all/download-template`, {
        responseType: 'blob',
      });
