...
const MainPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/jobdesc/${activeMenu.endpoint}/`);
        if (!response.ok) {
  ...
  const fetchJobs = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/jobdesc/${activeMenu.endpoint}/all/search?search=${searchTerm}`
      );
  ...
  const handleFileDownload = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/jobdesc/${activeMenu.endpoint}/all/download`, {
        responseType: 'blob',
      });
  ...
  const handleFileDownloadTemplate = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/jobdesc/${activeMenu.endpoint}/all/download-template`, {
        responseType: 'blob',
      });
